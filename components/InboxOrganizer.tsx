...
function Card({item,icon,onIcon,onOpen}:{item:InboxItem;icon:string;onIcon:()=>void;onOpen:()=>void}) {
...
export default function InboxOrganizer() {
  ...
  const saveIcon = (item:InboxItem, icon?:string) => {
    ...
    setIcons(next); localStorage.setItem(ICON_KEY,JSON.stringify(next)); setIconPicker(null);
  };
  const card = (item:InboxItem) => <Card key={item.id} item={item} icon={icons[item.id]||placeEmoji(item.title,item.placeType)} onIcon={()=>setIconPicker(item)} onOpen={()=>openOriginal(item)}/>;
  ...
  return createPortal(<>
    ...
    <section className="tf-organizer">
      ...
      {view==='all'&&<div className="tf-organizer-list">{items.map(card)}{!items.length&&<p className="tf-bucket-empty">아직 Inbox 후보가 없어요.</p>}</div>}
      {view==='region'&&<div>{groupedRegions.map(([region,list])=><section className="tf-group" key={region}><div className="tf-group-head"><h3>📍 {region}</h3><span>{list.length}곳</span></div><div className="tf-organizer-list">{list.map(card)}</div></section>)}</div>}
      {view==='bucket'&&<div><button className="tf-add-bucket" onClick={()=>setNewBucket(v=>!v)}><Plus size={16}/>버킷 먼저 만들기</button>{newBucket&&<div className="tf-new-bucket"><select value={bucketEmoji} onChange={e=>setBucketEmoji(e.target.value)}>{DEFAULT_EMOJIS.map(emoji=><option key={emoji}>{emoji}</option>)}</select><input value={bucketName} onChange={e=>setBucketName(e.target.value)} placeholder="예: 스프카레" onKeyDown={e=>e.key==='Enter'&&addBucket()}/><button className="primary" onClick={addBucket}>버킷 만들기</button></div>}{tripBuckets.map(bucket=>{const list=items.filter(item=>meta[item.id]?.bucketIds?.includes(bucket.id));return <section className="tf-group" key={bucket.id}><div className="tf-group-head"><h3>{bucket.emoji} {bucket.name}</h3><span>{list.length}곳</span></div>{list.length?<div className="tf-organizer-list">{list.map(card)}</div>:<div className="tf-bucket-empty">아직 후보가 없어요.<br/>장소를 찾은 뒤 이 버킷에 넣어주세요.</div>}</section>})}{!tripBuckets.length&&!newBucket&&<div className="tf-bucket-empty"><FolderHeart size={25}/><br/>“스프카레”, “야경”처럼<br/>하고 싶은 것부터 만들어보세요.</div>}</div>}
    </section>
