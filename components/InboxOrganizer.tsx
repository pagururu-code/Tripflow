...
export default function InboxOrganizer() {
  ...
  const card = (item:InboxItem) => <Card key={item.id} item={item} icon={icons[item.id]||placeEmoji(item.title,item.placeType)} onIcon={()=>{setIconPicker(item);setShowCustomIcon(false);}} onOpen={()=>openOriginal(item)}/>;
